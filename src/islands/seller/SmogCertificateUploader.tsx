...
import { updateSmogCertificateUrls, uploadDocument } from '../../lib/seller-api';

interface SmogCertificateUploaderProps {
  vehicleId: string;
  urls: string[];
...
export default function SmogCertificateUploader({
  vehicleId,
  urls,
  onUrlsChange,
}: SmogCertificateUploaderProps) {
  ...
  const handleFiles = async (files: FileList | File[]) => {
    ...
    try {
      for (const file of fileArray) {
        ...
        const url = await uploadDocument(vehicleId, file, 'smog_certificate');
        nextUrls.push(url);
        onUrlsChange([...nextUrls]);
      }
    } catch (err) {
      console.error('Smog certificate upload failed', err);
  ...
  const handleRemove = async (urlToRemove: string) => {
    ...
    try {
      await updateSmogCertificateUrls(vehicleId, nextUrls);
      onUrlsChange(nextUrls);
    } catch (err) {
      console.error('Smog certificate remove failed', err);
  ...
  return (
    <div className="mb-6">
      <div className="mb-4 rounded-lg bg-green-50 p-4 text-sm text-slate-700">
        Upload your recent Smog Check or Emissions Inspection Report. Most states require a passing
        smog check within 90 days of a private party sale. Upload a photo or PDF of your passing
        paperwork to give buyers peace of mind.
      </div>

      <label className="mb-2 block text-sm font-medium text-slate-700">
        Smog & Emissions Certificate
      </label>
          ...
          <>
            ...
            <p className="mb-1 text-sm font-medium text-slate-900">
              {urls.length > 0
                ...
                : 'Click or drag to upload smog certificate'}
            </p>
